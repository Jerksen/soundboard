...
class Profile():
    ...
    def load(self):
        ...
        # load the wavs for the soundboards
        for sub in os.listdir(os.path.join(self.dir, 'soundboard')):
            for i, w in enumerate(sorted(self.loadwavnames(os.path.join(self.dir, 'soundboard', sub)))):
                self.setup_db("sb{}".format(i), [w], sub)
    ...
    def loadwavnames(self, dirname):
        ...
        for f in ls:
            ...
            sounds.append(f)
            self.lgr.info("found {} to load in profile".format(f))
    ...
    def getfname(self, cat, wav, subcat=""):
        ...
        if cat[:2] == "sb":
            return os.path.join(self.dir, "soundboard", subcat, wav)
        else:
            return os.path.join(cat, wav)
